...
impl Target {
    pub fn parse(input: &str) -> Result<Self> {
            ...
            && let Ok(url) = Url::parse(input)
        {
            ...
            log::debug!(
                "[target] parsed_as_url: domain={:?} port={:?} scheme={:?}",
                target.domain,
                target.port,
                target.scheme
            );
    ...
    pub async fn resolve(&mut self) -> Result<&[IpAddr]> {
        log::debug!(
            "[target] resolve: domain={:?} cached_ips={}",
            self.domain,
        ...
        if self.resolved_ips.is_empty() && self.domain.is_some() {
            ...
            match self.resolve_domain().await {
                Ok(ips) => {
                    ...
                    log::trace!(
                        "[target] DNS resolution completed: domain={:?} ips={:?} duration={}ms",
                        self.domain,
                        self.resolved_ips,
                        resolve_duration.as_millis()
                    );
                }
                Err(e) => {
                    let resolve_duration = resolve_start.elapsed();
                    log::error!(
                        "[target] DNS resolution failed: domain={:?} duration={}ms error={}",
                        self.domain,
                        resolve_duration.as_millis(),
                        e
                    );
    ...
    pub fn primary_ipv4(&self) -> Option<Ipv4Addr> {
        let ipv4 = self.resolved_ips.iter().find_map(|ip| match ip {
    ...
    pub fn primary_ipv6(&self) -> Option<Ipv6Addr> {
        let ipv6 = self.resolved_ips.iter().find_map(|ip| match ip {
            IpAddr::V4(_) => None,
    ...
    pub fn update_resolved_ips(&mut self, ips: Vec<IpAddr>) {
        log::debug!(
            "[target] update_resolved_ips: old_count={} new_count={}",
            self.resolved_ips.len(),
            ips.len()
        );
        self.resolved_ips = ips;
    ...
    pub fn add_resolved_ips(&mut self, mut ips: Vec<IpAddr>) {
        ...
        log::debug!(
            "[target] add_resolved_ips: existing_count={} adding_count={}",
            old_count,
            ips.len()
        );
        ...
        log::debug!(
            "[target] add_resolved_ips_result: final_count={} added={}",
            new_count,
            new_count - old_count
        );
    ...
    pub fn ips_for_protocol(&self, protocol: Protocol) -> Vec<IpAddr> {
        let ips = match protocol {
            Protocol::Ipv4 => self.resolved_ips.iter().filter(|ip| ip.is_ipv4()).copied().collect(),
            Protocol::Ipv6 => self.resolved_ips.iter().filter(|ip| ip.is_ipv6()).copied().collect(),
            Protocol::Both => self.resolved_ips.clone(),
        };

        log::debug!(
            "[target] ips_for_protocol: protocol={} total_ips={} filtered_ips={}",
            protocol.as_str(),
            self.resolved_ips.len(),
            ips.len()
        );
    ...
    pub fn primary_ip_for_protocol(&self, protocol: Protocol) -> Option<IpAddr> {
        ...
        log::debug!(
            "[target] primary_ip_for_protocol: protocol={} ip={:?}",
            protocol.as_str(),
            ip
        );
        ip
    ...
    pub fn network_target_for_protocol(&self, protocol: Protocol) -> Option<String> {
        ...
        log::debug!(
            "[target] network_target_for_protocol: protocol={} target={:?}",
            protocol.as_str(),
            target
        );
        target
    ...
    pub fn supports_protocol(&self, protocol: Protocol) -> bool {
        ...
        log::debug!(
            "[target] supports_protocol: protocol={} supports={}",
            protocol.as_str(),
            supports
        );
        supports
    ...
    pub fn ip_count_for_protocol(&self, protocol: Protocol) -> usize {
        ...
        log::debug!(
            "[target] ip_count_for_protocol: protocol={} count={}",
            protocol.as_str(),
            count
        );
        count
    ...
    pub fn display_name_with_protocol(&self, protocol: Protocol) -> String {
        ...
        log::debug!(
            "[target] display_name_with_protocol: protocol={} name={}",
            protocol.as_str(),
            name
        );
        name
    ...
    pub fn ipv4_ips(&self) -> Result<Vec<IpAddr>> {
        let ipv4_ips: Vec<IpAddr> = self.resolved_ips.iter().filter(|ip| ip.is_ipv4()).copied().collect();

        if ipv4_ips.is_empty() {
            log::debug!(
                "[target] ipv4_ips: no IPv4 addresses available for {}",
                self.display_name()
            );
        ...
        log::debug!(
            "[target] ipv4_ips: found {} IPv4 addresses for {}",
            ipv4_ips.len(),
            self.display_name()
        );
        Ok(ipv4_ips)
    ...
    pub fn ipv6_ips(&self) -> Result<Vec<IpAddr>> {
        let ipv6_ips: Vec<IpAddr> = self.resolved_ips.iter().filter(|ip| ip.is_ipv6()).copied().collect();

        if ipv6_ips.is_empty() {
            log::debug!(
                "[target] ipv6_ips: no IPv6 addresses available for {}",
                self.display_name()
            );
        ...
        log::debug!(
            "[target] ipv6_ips: found {} IPv6 addresses for {}",
            ipv6_ips.len(),
            self.display_name()
        );
        Ok(ipv6_ips)
    ...
    pub fn primary_ipv4_ip(&self) -> Result<IpAddr> {
        match self.primary_ipv4() {
            ...
            None => {
                log::debug!(
                    "[target] primary_ipv4_ip: no IPv4 address available for {}",
                    self.display_name()
                );
    ...
    pub fn primary_ipv6_ip(&self) -> Result<IpAddr> {
        match self.primary_ipv6() {
            ...
            None => {
                log::debug!(
                    "[target] primary_ipv6_ip: no IPv6 address available for {}",
                    self.display_name()
                );
    ...
    pub fn ipv4_network_target(&self) -> Result<String> {
        if let Some(ip) = self.primary_ipv4() {
            ...
            log::debug!(
                "[target] ipv4_network_target: found {} for {}",
                target,
                self.display_name()
            );
            Ok(target)
            ...
            log::debug!(
                "[target] ipv4_network_target: using domain {} for {}",
                domain,
                self.display_name()
            );
            Ok(domain.clone())
            ...
            log::debug!(
                "[target] ipv4_network_target: no IPv4 target available for {}",
                self.display_name()
            );
    ...
    pub fn ipv6_network_target(&self) -> Result<String> {
        if let Some(ip) = self.primary_ipv6() {
            let target = ip.to_string();
            log::debug!(
                "[target] ipv6_network_target: found {} for {}",
                target,
                self.display_name()
            );
            Ok(target)
            ...
            log::debug!(
                "[target] ipv6_network_target: using domain {} for {}",
                domain,
                self.display_name()
            );
            Ok(domain.clone())
            ...
            log::debug!(
                "[target] ipv6_network_target: no IPv6 target available for {}",
                self.display_name()
            );
    ...
    async fn resolve_domain(&self) -> Result<Vec<IpAddr>> {
        ...
        let addresses = match lookup_host(&lookup_addr).await {
            Ok(addrs) => {
                ...
                log::trace!(
                    "[target] DNS lookup succeeded: duration={}ms",
            ...
            Err(e) => {
                let lookup_duration = lookup_start.elapsed();
                log::error!(
                    "[target] DNS lookup failed: domain={} duration={}ms error={}",
                    domain,
                    lookup_duration.as_millis(),
                    e
                );
...
mod tests {
    ...
    fn test_dual_stack_target() {
        ...
        assert_eq!(
            target.primary_ipv6(),
            Some(Ipv6Addr::new(0x2606, 0x2800, 0x220, 0x1, 0x248, 0x1893, 0x25c8, 0x1946))
        );
        ...
        assert_eq!(
            target.ipv6_addresses(),
            vec![Ipv6Addr::new(0x2606, 0x2800, 0x220, 0x1, 0x248, 0x1893, 0x25c8, 0x1946),]
        );
    ...
    fn test_protocol_specific_methods() {
        ...
        assert_eq!(
            target.primary_ip_for_protocol(Protocol::Ipv4),
            Some(IpAddr::V4(Ipv4Addr::new(93, 184, 216, 34)))
        );
        assert_eq!(
            target.primary_ip_for_protocol(Protocol::Ipv6),
            Some(IpAddr::V6(Ipv6Addr::new(
                0x2606, 0x2800, 0x220, 0x1, 0x248, 0x1893, 0x25c8, 0x1946
            )))
        );
        assert_eq!(
            target.primary_ip_for_protocol(Protocol::Both),
            Some(IpAddr::V4(Ipv4Addr::new(93, 184, 216, 34)))
        );
        ...
        assert_eq!(
            target.network_target_for_protocol(Protocol::Ipv4),
            Some("93.184.216.34".to_string())
        );
        assert_eq!(
            target.network_target_for_protocol(Protocol::Ipv6),
        ...
        assert_eq!(target.display_name_with_protocol(Protocol::Ipv4), "example.com (IPv4)");
        assert_eq!(target.display_name_with_protocol(Protocol::Ipv6), "example.com (IPv6)");
        assert_eq!(target.display_name_with_protocol(Protocol::Both), "example.com");
    }
    ...
    fn test_protocol_specific_methods_unresolved_domain() {
        ...
        assert_eq!(
            target.network_target_for_protocol(Protocol::Both),
            Some("unresolved.example".to_string())
        );
    }
    ...
    fn test_explicit_protocol_methods_success() {
        ...
        assert_eq!(
            target.primary_ipv6_ip().unwrap(),
            IpAddr::V6(Ipv6Addr::new(0x2606, 0x2800, 0x220, 0x1, 0x248, 0x1893, 0x25c8, 0x1946))
        );

        assert!(target.ipv6_network_target().is_ok());
        assert_eq!(
            target.ipv6_network_target().unwrap(),
